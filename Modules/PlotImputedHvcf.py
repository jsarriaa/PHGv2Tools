def PlotHvcf():
    ...
    parser = ArgumentParser(description=f'{PlotHvcf.__doc__}')
    parser.add_argument('-hvcf', '--hvcf-file', help='Input HVCF file', required=True)
    parser.add_argument('-plot', '--plot-folder', help='Folder to save the plot', required=True)
    parser.add_argument('-db', '--pangenome-hvcfs-database', help='Pangenome HVCFs database', required=True)
    parser.add_argument('-ref', '--reference-hvcf', help='Reference HVCF file', required=True)
    args = parser.parse_args()

    hvcf = args.hvcf_file
    plot_folder = args.plot_folder
    pangenome_hvcfs_database = args.pangenome_hvcfs_database
    reference_hvcf = args.reference_hvcf
        
    hvcf_files_path = [os.path.join(pangenome_hvcfs_database, file) for file in os.listdir(pangenome_hvcfs_database) if file.endswith('.h.vcf.gz')]
    ...
    def BEDSplitByChr (hvcf, plot_folder):
        ...
        chr_list = CaptureChrFromBED(hvcf)
    ...
    def RunPygenometracksCommandHVCF(hvcf, reference_name, plot_folder, genomes):
        ...
        command = (f"pyGenomeTracks --tracks {track_ini} --region {os.path.basename(hvcf).split('.')[0]}:1-{chr_max} --outFileName {plot_folder}/{os.path.basename(hvcf).split('.')[0]}.png --width {width} --dpi 500")
    #   command = (f"pyGenomeTracks --tracks {track_ini} --region {os.path.basename(hvcf).split('.')[0]}:1-{chr_max} --outFileName {plot_folder}/{os.path.basename(hvcf).split('.')[0]}.png --width {width} --height 10")
    ...
    def AddLegendToPygenometracks (hvcf, plot_folder, genomes, reference_name):
        ...
        # Open the main plot and the legend
        plot_image = Image.open(f'{plot_folder}{os.path.basename(hvcf).split('.')[0]}.png')
        ...
        plot_image.save(f'{plot_folder}{os.path.basename(hvcf).split('.')[0]}.png')

        print ("Done! The legend is added to the plot\n")
        print(f"Your plot is saved at {plot_folder}{os.path.basename(hvcf).split('.')[0]}.png")
