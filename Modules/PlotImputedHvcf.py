...
def hvcf2bed(imputed_hvcf, plot_folder, colors_dic):
    """
    ...
    *Notice that the new files WILL NOT CONTAIN in each line to which genome is the range belonging to.
    ...
    hvcf_BED = (f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}.bed")

    if os.path.exists(hvcf_BED):
        #print(f"File {hvcf_BED} already exists, removing it and building it again") 
        os.remove(hvcf_BED)

    print(f"Converting {imputed_hvcf} to BED file\n")

    if imputed_hvcf.endswith('.gz'):                        ##Checks if the file is compressed or not and acts depending on it
        open_func = lambda f: gzip.open(f, 'rt')
    else:
        open_func = open

    with open_func(imputed_hvcf) as hvcf_file, open(hvcf_BED, 'w') as bed_file:
        first_line = True
        ...
        for line in lines:
            ...
            if line.startswith ('##ALT=<ID'):
                ...
                if first_line == True:
                    ...
                    first_line = False
                    bed_file.write(f"{os.path.basename(imputed_hvcf).split('.')[0]}\t{start}\t{end}\t{chr}\t100\t+\t{start}\t{end}\t{colors_dic[name]}\n")
                    #print(line)
                else:
                    bed_file.write(f"{os.path.basename(imputed_hvcf).split('.')[0]}\t{start}\t{end}\t{chr}\t100\t+\t{start}\t{end}\t{colors_dic[name]}\n")
                    #print(chr)
        ...
        print(f"finished processing {imputed_hvcf} to BED file {hvcf_BED}\n")
...
def CaptureChrFromBED (imputed_hvcf, plot_folder):
    """
...
def BEDSplitByChr (imputed_hvcf, plot_folder):
    pass
    """
    ...
    chr_list = CaptureChrFromBED(imputed_hvcf, plot_folder)
    ...
    for chr in chr_list:    #check if the files splited by chr already exist, and if they do, remove them            
        chr_file = (f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}_{chr}.bed")
...
def sortBED (imputed_hvcf, plot_folder):
...
#Now sort the files by start position
    for chr in chr_list:
        chr_file = (f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}_{chr}.bed")
        sorted_chr_file = (f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}_{chr}_sorted.bed")
...
def PrepareTrack_Hvcf2plot(imputed_hvcf, reference_name, plot_folder):
    """
    This function will prepare the tracks for pygenometracks, so it can plot the imputed_hvcf files. Indicate the hvcf initial file, and the reference name, as long
    ...
    track_ini = (f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}_track.ini")
    ...
    with open (track_ini, 'w') as track:
        ...
        for chr in chr_list:
            ...
            track.write(f"file = {plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}_{chr}_sorted.bed \n")
...
def SetExtremesToPlot(imputed_hvcf, plot_folder):
    """
    """    
    #capturing the extreme values to plot of each chromosome
    chr_list = CaptureChrFromBED(imputed_hvcf, plot_folder)
    print(f"the chromosomes to plot are {chr_list}\n")
    chr_max = None
    for chr in chr_list:
        chr_file = (f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}_{chr}_sorted.bed")
        with open (chr_file, 'r') as f:
...
def RunPygenometracksCommandHVCF(imputed_hvcf, reference_name, plot_folder, genomes):
    ...
    chr_max = SetExtremesToPlot(imputed_hvcf, plot_folder)
    track_ini = PrepareTrack_Hvcf2plot(imputed_hvcf, reference_name, plot_folder)
    chr_list = CaptureChrFromBED(imputed_hvcf, plot_folder)

    width = len(genomes)*12
    height = len(chr_list)*10

    command = (f"pyGenomeTracks --tracks {track_ini} --region {os.path.basename(imputed_hvcf).split('.')[0]}:1-{chr_max} --outFileName {plot_folder}/{os.path.basename(imputed_hvcf)}.png --width {width} --dpi 500")
    ...
    print(f"the plot is saved at {plot_folder}{os.path.basename(imputed_hvcf).split('.')[0]}.png\n")


# In[70]:


def AddLegendToPygenometracks (imputed_hvcf, plot_folder, genomes, reference_name):
    """
    ...
    # Check if the reference genome is in the pangenome, if not, do not include it at legend
    with open (f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}.bed", 'r') as f:
        lines = f.readlines()
        ref_in_pangenome = False
        for line in lines:
            variety = line.split('\t')[0]
            if variety == reference_name:
                ref_in_pangenome = True
                break
        if ref_in_pangenome == False:
    ...
    plot_image = Image.open(f'{plot_folder}{os.path.basename(imputed_hvcf)}.png')
    ...
    plot_image.save(f'{plot_folder}{os.path.basename(imputed_hvcf)}.png')

    print ("Done! The legend is added to the plot\n")
    print(f"Your plot is saved at {plot_folder}{os.path.basename(imputed_hvcf)}.png")
...
def main(pangenome_hvcf_folder, imputed_hvcf, reference_hvcf):
    """
    ...
    plot_folder = (f"{pangenome_hvcf_folder}/plots/")
    hvcf_files_path = [os.path.join(pangenome_hvcf_folder, file) for file in os.listdir(pangenome_hvcf_folder) if file.endswith('.h.vcf.gz')]
    ...
    print(f"\nYour reference hvcf file is: \n{reference_hvcf}\n \nThe reference genome is: \n{reference_name}\n ")

    print(f"the plot folder is {plot_folder}\n")  

    colors_dic, ref_color = assign_colors(genomes, reference_name)

    hvcf2bed(imputed_hvcf, plot_folder, colors_dic)

    BEDSplitByChr (imputed_hvcf, plot_folder)

    sortBED (imputed_hvcf, plot_folder)

    RunPygenometracksCommandHVCF(imputed_hvcf, reference_name, plot_folder, genomes)

    AddLegendToPygenometracks (imputed_hvcf, plot_folder, genomes, reference_name)

    #Remove temporary files
    chr_list = CaptureChrFromBED(imputed_hvcf, plot_folder)
    os.remove(f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}.bed")
    os.remove(f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}_track.ini")
    os.remove(f"{plot_folder}TEMP_legend.png")
    for chr in chr_list:
        os.remove(f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}_{chr}.bed")
        os.remove(f"{plot_folder}TEMP_{os.path.basename(imputed_hvcf).split('.')[0]}_{chr}_sorted.bed")
